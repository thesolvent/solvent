//! Shared UniswapX V1/V2 wire types and their `orderHash`es. Both hashes reproduce their reactor's
//! own lib (`DutchOrderLib`/`V2DutchOrderLib`): the type strings flatten nested structs, so alloy's
//! derived hashing can't produce them — composed instead from the contracts' own type strings and
//! alloy's `abi_encode`. `OrderInfo`/`DutchOutput` and their hashing are shared across both, since
//! every UniswapX reactor generation uses the same two.
...
pub(crate) const V1_DUTCH_ORDER_TYPE: &[u8] = b"ExclusiveDutchOrder(OrderInfo info,uint256 decayStartTime,uint256 decayEndTime,address exclusiveFiller,uint256 exclusivityOverrideBps,address inputToken,uint256 inputStartAmount,uint256 inputEndAmount,DutchOutput[] outputs)";
...
/// `ExclusiveDutchOrderLib.hash` — the Orders API's `Limit` order type, on the original
/// (non-V2) `ExclusiveDutchOrderReactor`: exclusivity is a plain field the swapper signed, not a
/// cosigner override, so there is no separate pre-override/post-override amount to reconcile the
/// way `order_hash` does for V2. Same field-hashing helpers, since `OrderInfo`/`DutchOutput` are
/// shared across every UniswapX reactor generation.
pub(crate) fn v1_order_hash(order: &V1DutchOrder) -> B256 {
    let order_type_hash =
        keccak256([V1_DUTCH_ORDER_TYPE, DUTCH_OUTPUT_TYPE, ORDER_INFO_TYPE].concat());
    keccak256(
        (
            order_type_hash,
            info_hash(&order.info),
