...
impl LedgerStore for SqliteLedgerStore {
    ...
    async fn open_reservations(&self) -> Result<Vec<Reservation>, LedgerStoreError> {
        let rows: Vec<(Vec<u8>, Vec<u8>, String, String, i64)> = sqlx::query_as(
            "SELECT id, intent, sources, state, expires_at FROM ledger_reservation
        ...
        rows.into_iter()
            .map(|(id, intent, sources, state, expires_at)| {
