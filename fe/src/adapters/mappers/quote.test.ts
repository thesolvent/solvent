...
describe("toQuote", () => {
  it("scales the raw output by the token's own decimals", () => {
    ...
    expect(toQuote(fixture, 6).amountOut).toBe("2,477.85");
  });

  it("prints more places when the output is small", () => {
    expect(toQuote(fixture, 18).amountOut).toBe("0.000000");
  });

  it("reports the output's own USD value, not the input's", () => {
    expect(toQuote(fixture, 6).amountOutUsd).toBe(fixture.amount_out.usd);
  });
});
