...
const { Strategy, linearWidthFromSymmetricRangePercent, positions } =
  createRequire(import.meta.url)(
    "@solvent/sdk",
  ) as typeof import("@solvent/sdk");

const env = await connectDevnet().catch((error) => {
  console.error(failureMessage(error));
  process.exit(1);
});
const { api, publicClient } = env;
...
async function seedPair(
  ...
  for (const leg of [sized.base, sized.quote]) {
    await maker.mint(leg.address, parseUnits(String(MINT_UNITS), leg.decimals));
